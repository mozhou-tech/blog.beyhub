...
1. 如果查询条件中有OR，这相关字段都要加索引；否则条件中有带索引的字段也不会生效 

2. 对于联合索引，基于最左优先匹配原则，只有左侧第一个索引可以单独生效 

3. 对于Like查询，%在最左侧时索引失效，%在最右侧是索引生效 
4.  如果字段类型是字符串，那条件中的值应使用引号引用起来，否则不使用索引 
5. 如果MySQL的优化器评估全表扫描比索引快，则不使用索引 
6. 索引列是字符串时，如果传入的条件参数是整数，会先转换成浮点数，再全表扫描，导致索引失效； 
7. 条件参数要尽可能与列的类型相同，避免隐式转换，或者把传入的条件参数转换成索引列的类型 
8. 查询条件使用is null，is not null，索引失效 9. 查询条件使用不等于（!= 或<>），索引失效 
9. 在索引字段上进行计算、函数、类型转换等内置函数使索引失败 
10. 索引字段上使用（！= 或者 < >，not in）时，可能会导致索引失效。  

索引失效的原因：索引失效的原因是优化器发现不能利用索引的有序性 

隐式类型转换：本质上是对索引字段使用了CAST()函数，以实现不同类型之间的比较
